    ...
    <div class="row">
        ...
        <div class="col-md-8 pl-0">
            <div class="card card-outline card-orange ml-1">
                ...
                <h3 class="card-title"><i class="fa fa-linode"></i> &nbsp;{{ __('library.Library List') }}</h3>
        ...
        <div class="col-md-4 pr-0">
            <div class="card card-outline card-orange mr-1">
                 <div class="card-header bg-primary">
                <h3 class="card-title"><i class="fa fa-edit"></i> &nbsp;{{ __('library.Edit Library') }} </h3>
                <div class="card-tools">
                <a href="{{url('library_add')}}" class="btn btn-primary  btn-sm" ><i class="fa fa-plus"></i>{{ __('common.Add') }} </a>
                <!--<a href="{{url('students_dashboard')}}" class="btn btn-primary  btn-sm" ><i class="fa fa-arrow-left"></i> Back</a>-->
                </div>
                
                </div>                 
                <form id="quickForm" action="{{ url('library_edit') }}/{{ $data['id'] ?? '' }}" method="post">
                @csrf
                <div class="row m-2">
                    <div class="col-md-12">
            			<label style="color:red;">{{ __('library.Library Name') }}*</label>
            			<input class="form-control @error('name') is-invalid @enderror" type="text" name="name" id="name" placeholder="{{ __('library.Library Name') }}" value="{{ $data['name'] ?? '' }}">
                         @error('name')
