				...
				<div class="col-12 col-md-12">
					<div class="card card-outline card-orange">
						<div class="card-header bg-primary">
							<h3 class="card-title"><i class="fa fa-address-book-o"></i> &nbsp;{{ __('hostel.Hostel Expense Add') }}</h3>
						...
						<form id="quickForm" action="{{ url('hostelExpensesAdd') }}" method="post" enctype="multipart/form-data">
							@csrf
							...
							<div class="row m-2">
								<div class="col-md-3">
									<div class="form-group">
										...
										<select class="form-control @error('expense_head') is-invalid @enderror select2" id="expense_head" name="expense_head">
								...
								<div class="col-md-3">
									<div class="form-group">
										...
										<input type="text" class="form-control @error('expense_name') is-invalid @enderror" id="expense_name" name="expense_name" placeholder="{{ __('hostel.Expense Name') }}" value="{{old('expense_name')}}">
										@error('expense_name')
										<span class="invalid-feedback" role="alert">
											<strong>{{ $message }}</strong>
										</span>
								...
								<div class="col-md-3">
									<div class="form-group">
										...
										<input type="date" class="form-control @error('expense_date') is-invalid @enderror" id="expense_date" name="expense_date" placeholder="{{ __('messages.Last Name') }}" value="{{date('Y-m-d')}}">
										@error('expense_date')
										<span class="invalid-feedback" role="alert">
											<strong>{{ $message }}</strong>
										</span>
								...
								<div class="col-md-3">
									<div class="form-group">
										...
										<input type="text" class="form-control @error('expense_amount') is-invalid @enderror" id="expense_amount" name="expense_amount" placeholder="{{ __('hostel.Expense Amount') }}" value="{{old('expense_amount')}}" maxlength="10" onkeypress="javascript:return isNumber(event)">
								...
								<div class="col-md-3">
									<div class="form-group">
										...
										<select class="form-control @error('expense_by') is-invalid @enderror select2" id="expense_by" name="expense_by">
											<option value="">{{ __('common.Select') }}</option>
											@if(!empty($getAllUsers))
											@foreach($getAllUsers as $value)
											<option value="{{ $value->id}}" {{ ($value->id == old('expense_by')) ? 'selected' : '' }}>{{ $value->first_name ?? ''  }}{{ $value->last_name ?? ''  }}</option>
											@endforeach
											@endif
										</select>
										@error('expense_by')
	                               ...
	                               <div class="col-md-3">
									<div class="form-group">
										...
										<select class="form-control @error('payment_mode') is-invalid @enderror select2" id="payment_mode" name="payment_mode">
											<option value="">{{ __('common.Select') }}</option>
