        					...
        					<tbody>
            					...
            					@foreach ($data as $key1=>$type)
            						...
            						<tr>
            							...
            							<td>{{date('d-m-Y', strtotime($type['submission_date'])) ?? '' }}
            							@if(Session::get('role_id') == 3)
                                        ...
                                        <td class="row">
                                            @if(!empty($data)) 
                                                @foreach($data as $key=>$info)
                                                    ...
                                                    <img src="{{ env('IMAGE_SHOW_PATH').'uploadHomework/'.$info->student_upload_file }}" width="60px" height="60px" onerror="this.src='{{ env('IMAGE_SHOW_PATH').'/default/user_image.jpg' }}'">
                                                    <span class="">
                                                    <a href="{{ url('download_assignment') }}/{{$info->student_upload_file }}" class="btn btn-primary  btn-xs ml-3" title="Download Assignments" ><i class="fa fa-download"></i></a>
                                                    <a data-href="{{ env('IMAGE_SHOW_PATH').'uploadHomework/'.$info->student_upload_file }}" data-upload_id="{{$info['upload_hw_id']}}"  class="viewModal_{{$info['upload_hw_id']}}  viewModal2 btn btn-warning btn-xs" title="View Assignment" ><i class="fa fa-eye"></i></a>     
                                                    </span>
