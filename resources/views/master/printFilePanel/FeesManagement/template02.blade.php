...
<div class="col-sm-3 border-right"><b>:{{ $data[0]['Admission']['first_name'] ?? '' }} {{ $data[0]['last_name'] ?? '' }}</b></div>
<div class="col-sm-2"><b>Father's Name</b></div>
<div class="col-sm-3"><b>: {{ $data[0]['Admission']['father_name'] ?? '' }}</b></div>
...
<div class="col-sm-3 border-right"><b>: {{ $data[0]['Admission']['ClassTypes']['name'] ?? '' }}

</b>
</div>
<div class="col-sm-2"><b>Mobile</b></div>
<div class="col-sm-3"><b>: {{ $data[0]['Admission']['mobile'] ?? '' }}</b></div>
</div>
<div class="row">
...
<div class="col-sm-3 border-right">
    @php
                    ...
                    @endphp<b>: {{ date('F', strtotime($fees_master->installment_due_date)) ?? '' }}</b></div>
 @endforeach
  @endif
</div>
<div class="row">
...
<table style="width:100%;" class="table-bordered">
    ...
    <tr style="border-top:2px solid black;">
        <td colspan="3">{{$data[0]['receipt_no'] ?? ''}}</td>
        <td colspan="3">{{ date('d-M-Y', strtotime($data[0]->date)) }}</td>
        <td colspan="3">{{ $data['discount_value'] ?? '-' }}</td>
        <td colspan="3">
              @php
            ...
            {{ number_format($fees_master->amount ?? 0, 2) }}</td>
            <td>  
            @php
...
<td colspan="3"></td>
<td colspan="3"></td>
<td colspan="3"><b>: {{ $payment_name->name ?? '' }}</b></td>
...
<td colspan="3" ><b>:</b> <b id="inNumber">
    ...
    {{ number_format ($total_amount ?? 0, 2)  }}</b></td>
...
<td style="width:14%;"></td>
  @php
                ...
                $words .= ' rupees';
                @endphp
<td style="width: 32%;"><b id="inWords"> : {{ $words}}</b></td>
