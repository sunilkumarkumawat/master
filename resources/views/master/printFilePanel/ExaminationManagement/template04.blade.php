...
<body>
    ...
    @foreach($admission_id as $admission_id)
	    <table style="border: 2px solid;background-color: #6639b5;color: white;">
			<tbody >
					<tr>
      <td  rowspan="4">
          <img  rowspan="4" src="{{ env('IMAGE_SHOW_PATH').'/setting/left_logo/'.$getSetting['left_logo'] }}" style="width: 130px;" onerror="this.src='{{ env('IMAGE_SHOW_PATH').'/default/rukmani_logo.png' }}'">
      </td>
    ...
    @endphp
      ...
      <h6 class="text-center"><b class="back_bg"><span style="color:red;">Term-1</span> Academic Performance - Session {{$session->from_year ?? '' }}{{"-"}}{{$session->to_year ?? ''}}</b></h6>
...
<thead>
 ...
 <table>
  ...
  <table class="mt-2" style="border: 2px solid;width: 100%;">
  ...
  <table class="mt-3" style="border: 2px solid;width: 100%;">
