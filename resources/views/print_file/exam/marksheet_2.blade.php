...
<body>
    ...
    $classOrderBy = DB::table('class_types')->where('id',$admission_id->class_type_id)->first();
    
    
    if($classOrderBy->orderBy > 10 )
    {
    
    if($admission_id->stream_subject != '')
    {
    $streams = explode(',', $admission_id->stream_subject);
    }
    }
...
<thead>
  ...
  @endphp
   ...
   //dd($item_subject);
   @endphp
      @if(in_array($item_subject->id, $streams) )
   @php
...
if ($student_marks == 'AB') {
 ...
 <table width='100%'>
  ...
  <table class="mt-2" style="border: 2px solid;width: 81%;">
			<tbody>
			<tr>
			    <th class=" AreaBorder"><b>Other Subjects</b> </th>
