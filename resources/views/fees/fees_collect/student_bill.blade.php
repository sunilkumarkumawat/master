    ...
    <div class="col-8 col-md-6">
        <div class='row' >
         ...
         <div class="p-2 col-3 col-md-9 text-right d-flex" style='justify-content: end; align-items: end;'>
            <h3> Receipt No.:<span class='text-danger'>{{$data['BillCounter']['counter']+1 ?? ''}}</span></h3>
             </div>   
        </div>
        <div class="card">
    ...
    <div class="col-md-3">
                        ...
                        <div class="col-md-3">
                            <label class="text-danger">{{ __('common.Amount') }}*</label>
                               <input type="hidden" class="form-control" placeholder="{{ __('fees.Receipt No.') }}" name="slip_no[]"  value="{{$data['BillCounter']['counter']+1 ?? ''}}" onkeypress="javascript:return isNumber(event)"  readonly>
                ...
                <div class="row pl-3 "> 
                    <div class="col-md-12 text-left">
                        <input type="hidden" name="message_type" value="{{ $permission_messages->type ?? '' }}">
                        <input type='checkbox' id='set_whatsapp_permission'name='sendReceipMedia' {{ ($permission_messages->permission == 1) ? 'checked' : '' }} />&nbsp; <label for='set_whatsapp_permission'>Send Fees Collection Confirmation Message On Whatsapp.</label><br>
                ...
                <div class="row p-2 "> 
                    <div class="col-md-12 text-center">
                        ...
                        <button type="submit" id="collect_btn"  name="print"  class="btn btn-primary collect_btn"><i class="fa fa-print"></i> {{ __('Collect & Print') }} </button>
                    </div>
                </div> 
                </form>
            </div>
                ...
                <button type="button"  class="btn btn-info" id="toggle-rows">Show</button>
                    ...
                    <table id='fee_structure'class="table table-bordered padding_table" style="white-space:nowrap;">
                        ...
                        <tbody>
                            ...
                            @endphp
                            <div class="col-md-12">
                                <tr id='group_{{$feesGroup->id ?? ''}}' class='group_group toggle-row' >
                                ...
                                <td id="pending_by_group_id_{{$feesGroup->id ?? ''}}" class="{{ $pending_amount == 0 ? 'bg-success ' : '' }} {{ $pending_amount == 0 ? 'hidden ' : '' }}"
    ...
    <div class="card-body">
        ...
        <table class="table table-bordered small_td p-3 " id="trColor">
                        <thead class='bg-primary'>
                            <tr>
                                <th><input type='checkbox' name='select_all' id='select_all'/ > {{ __('S.No') }}</th>
                     ...
                     @php
                        ...
                        <tr>
                            <td>
                                <input type='checkbox' data-admission_id="{{$item->admission_id ?? ''}}"  data-pending="{{$amount['total']-$amount['paid']}}" data-receipt='{{$item->receipt_no}}'name='checkbox_select' class='checkbox_select'/ > {{$key+1}}
                                
                            </td>
                            <td>
                              <a target='_blank'href="{{url('storeReceipt',$item->receipt_no)}}"class='btn btn-xs btn-primary'>    {{$item->receipt_no ?? ''}}</a>
                            </td>
                            <td>
                                {{$item->date ?? ''}}
                            </td>
                            ...
                            @endphp
                            <td class="bg-{{$pending > 0 ? 'danger' : ''}}">
                                ₹ {{($amount['total'] ?? 0) - ($amount['paid'] ?? 0)}}
                            </td>
                        </tr>
                        ...
                        <tfoot>
                            ...
                            <td >₹ {{$total_inventory_amount ?? 0}}</td>
                            <td >₹ {{$total_inventory_paid ?? 0}}</td>
                            <td >₹ {{$total_inventory_pending ?? 0}}</td>
                           </tr>
                        </tfoot>
                </table>
        ...
        @endphp
            <tr>
                ...
                <td>
                     ...
                     <input type='hidden' name='fees_invoice_id' value='{{$val->fees_details_id}}' />
                ...
                <td>
                  <button class="btn btn-danger btn-xs revert_fees" data-session_id="{{ $val->session_id ?? '' }}"  data-admission_id="{{ $val->admission_id ?? ''}}" data-id='{{ $val->id }}' data-toggle="modal" data-target="#revert_modal"><i class="fa fa-undo"></i></button>
                </td>
            </tr>
        @endforeach 
